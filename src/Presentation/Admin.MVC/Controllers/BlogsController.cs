...
namespace Admin.MVC.Controllers
{
    public class BlogsController : BaseController
    {
        ...
        #region ( Tags )
        #region ( Index )
        public IActionResult IndexTags(TagsFilterViewModel filter)
        {
        ...
        public async Task<IActionResult> CreateTag(CreateTagViewModel createTagViewModel)
        {
            if (!ModelState.IsValid)
            {
                ...
                ViewData["categories"] = await UnitOfWork.TagRepository.GetAllAsync();

                return RedirectToAction("IndexTags");
            }

            var tag = Mapper.Map<Tag>(createTagViewModel);

            var iconName = await FtpService.UploadFileToFtpServer(createTagViewModel.IconPictureFile, TypeFile.Image, Paths.CmsBlogTag, createTagViewModel.IconPictureFile.FileName);
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateTag(UpdateTagViewModel updateTagViewModel)
        {
            if (!ModelState.IsValid)
            {
                ErrorAlert(Errors.ModelStateIsNotValidForm);
            ...
            if (updateTagViewModel.IconPictureFile != null)
            {
                ...
                var iconName = await FtpService.UploadFileToFtpServer(updateTagViewModel.IconPictureFile, TypeFile.Image, Paths.CmsBlogTag, updateTagViewModel.IconPictureFile.FileName, 777, null, null, null, deletedFile);
